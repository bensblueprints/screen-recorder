...
// ── Debug log (write to appData, not __dirname which is inside asar) ──
const LOG_DIR = config.appDataDir;
...
let onUpdate = null;
let onFrame = null;   // callback for webcam preview frames
...
function buildScreenArgs(opts, outDir) {
  ...
  if (process.platform === 'win32') {
    // Capture full desktop, then crop to selected monitor via filter
    args.push('-probesize', '10M', '-thread_queue_size', '1024',
      '-f', 'gdigrab', '-framerate', String(config.framerate),
  ...
  // Crop to selected monitor if display info provided
  if (opts.display && opts.display.width && process.platform === 'win32') {
    args.push('-vf', `crop=${opts.display.width}:${opts.display.height}:${opts.display.cropX}:${opts.display.cropY}`);
  }
...
function buildWebcamArgs(opts, outDir) {
  ...
  // Output 1: recording file (H.264 + AAC)
  args.push('-map', '0:v', '-pix_fmt', 'yuv420p');
  args.push(...encoderArgs(opts.encoder));
  if (opts.audioDevice) {
    args.push('-map', '0:a', '-c:a', 'aac', '-b:a', config.audioBitrate);
  }
  args.push('-movflags', '+faststart', '-y', webcamFile);

  // Output 2: MJPEG preview stream to stdout (low-res, 15fps)
  args.push('-map', '0:v', '-c:v', 'mjpeg', '-q:v', '5',
    '-r', '15', '-s', '320x240', '-f', 'image2pipe', 'pipe:1');
...
function spawnFFmpeg(args, label) {
  return new Promise((resolve, reject) => {
    ...
    // Webcam uses stdout for preview frames, screen doesn't
    const useStdoutPipe = label === 'webcam';
    const proc = spawn(config.ffmpeg, args, {
      stdio: ['pipe', useStdoutPipe ? 'pipe' : 'ignore', 'pipe']
    });
    let stderrBuf = '';
    let exited = false;
...
// ── Start recording ──
async function start(opts, updateCallback, frameCallback) {
  ...
  // Pipe webcam stdout through pipe2jpeg for live preview frames
  if (webcamProc && webcamProc.stdout) {
    const p2j = new Pipe2Jpeg();
    webcamProc.stdout.pipe(p2j);
    p2j.on('data', (jpegBuf) => {
      if (onFrame) onFrame(jpegBuf);
    });
  }
...
function stackExport(sessionPath, encoder, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      '-filter_complex',
      [
        // Scale screen to fit 1080x960 box, pad with black to fill exactly 1080x960
        '[0:v]scale=1080:960:force_original_aspect_ratio=decrease,pad=1080:960:(ow-iw)/2:(oh-ih)/2:black,setsar=1[top]',
        // Scale webcam to fit 1080x960 box, pad with black to fill exactly 1080x960
